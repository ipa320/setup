...
Sick changed the their scanner product line. From now on (April 2013) the Sick Professional CMS, that we used so far, has been replaced by the Sick Expert.
...
Additionally, the new Scanner also have a new telegram protocoll version, which is not compatible with the current driver.
...
After that you can edit the configuration or import one (an example configuration for the new scanners can be found at GitHub\footnote{for the rob@work: \url{https://github.com/ipa320/setup/tree/master/backup_configuration/raw3-3/raw3-3_exert_front.sks}}).
